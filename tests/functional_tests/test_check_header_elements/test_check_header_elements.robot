...
${test_name}      check header elements
${test_tags}      Проверка наличия всех обязательных элементов хедера, на сайте
...
*** Keywords ***
# ОСНОВНОЙ СЦЕНАРИЙ ТЕСТА, по проверке всех обязательных элементов хедера.
...
Test run    # Общее ключевое слово, для всех тестов. Которое вызывается из листенера. Подключает в себе ключевое слово, конкретное для каждого теста.
    ...
    Check header elements     ${url}

Check header elements
    [Arguments]     ${url}
    Check all header elements      ${url}
